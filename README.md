...
(`kunrag/m2-php-7.2-fpm` и `kunrag/m2-php-7.2-fpm`). В папке docker лежит makefile и исходные настройки контейнеров.
Можно конфигурировать основные настройки как душе угодно, а затем запусить `make build`. Данна команда создаст обновленные контейнеры
